...
下载pytorch预训练模型，再运行如下脚本，得到对应的mindspore模型。注：运行该脚本需要同时安装pytorch环境(测试版本号为1.3，CPU 或 )
...
# 运行训练示例
bash run_standalone_train_.sh

# 运行分布式训练示例
# DEVICE_NUM: 使用的显卡数量，如: 8
# USED_DEVICES: 使用的显卡id列表，需和显卡数量对应，如: 0,1,2,3,4,5,6,7
bash run_distribute_train_.sh [DEVICE_NUM] [USED_DEVICES]
...
├── model_zoo
    ├── EGNet
        ...
        │   ├──run_standalone_train_.sh  # 启动单机训练（单卡）
        │   ├──run_distribute_train_.sh  # 启动分布式训练（多卡）
        │   ├──run_eval_.sh              # 启动评估
...
```text
device_target: "Ascend"                                 # 运行设备 ["Ascend", ""]
...
- 处理器环境运行，需修改default_config.yaml中的model参数为需要评估的模型路径
